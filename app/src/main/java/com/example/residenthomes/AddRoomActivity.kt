...
class AddRoomActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        threeSharing.setOnClickListener {
            selectedImageView = threeSharing
            Intent(Intent.ACTION_GET_CONTENT).also{
                it.type ="image/*"
                imageLauncher.launch(it)
        ...
        eightSharing.setOnClickListener {
            selectedImageView = eightSharing
            Intent(Intent.ACTION_GET_CONTENT).also{
                it.type ="image/*"
                imageLauncher.launch(it)
        ...
        if (result.resultCode == RESULT_OK){
            result?.data?.data?.let {
                ...
                when (selectedImageView) {
                    deposit -> deposit.setImageURI(it)
                    oneSharing -> oneSharing.setImageURI(it)
                    twoSharing -> twoSharing.setImageURI(it)
                    threeSharing -> threeSharing.setImageURI(it)
                    fourSharing -> fourSharing.setImageURI(it)
                    sixSharing -> sixSharing.setImageURI(it)
                    eightSharing -> eightSharing.setImageURI(it)
                    tenSharing -> tenSharing.setImageURI(it)
                }
            }
        }else{
